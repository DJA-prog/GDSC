...
if (isset($_POST["sumbit"])) { //check if submit button was pressed
  ...
  //print_r($file); // print the array called $file
  ...
  $fileExt = pathinfo($fileName, PATHINFO_FILENAME); // returns the file extention
  ...
  if (in_array($fileExt, $allowedFormats)) { // check if uploaded file is allowed
    if ($fileError === 0) { // === is the same as == but will check if it is related
      if ($fileSize < 1000000) { // allow for only file smaller than 1 000 000 KB
  ...
  header("Location: ./index.php"); //return to the previous page
